...
typedef RT_MODEL_GyroscopeHinfinityCo_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * GyroscopeHinfinityController(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(GyroscopeHinfinityController_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://GyroscopeHinfinityController:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: GyroscopeHinfinityController -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
   ...
   ************************/
  S = GyroscopeHinfinityController();
  if (rtmGetErrorStatus(S) != NULL) {
